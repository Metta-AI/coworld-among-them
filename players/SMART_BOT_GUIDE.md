# Among Them Policy Improvement Guide

This page is for policy authors or optimizers that already have a valid Among
Them player image and want to improve it. Start with the hosted contract, then
improve perception, control, and strategy in that order.
...
`COWORLD_PLAYER_WS_URL` and `COGAMES_ENGINE_WS_URL`.

The policy must:

1. connect to that URL exactly;
2. decode Bitscreen v1 frames;
3. send valid button and chat packets;
...
- Bitscreen protocol: <https://github.com/Metta-AI/bitworld/blob/master/docs/bitscreen_v1.md>
- Replay/global protocol: <https://github.com/Metta-AI/bitworld/blob/master/docs/sprite_v1.md>
- Play guide: <https://softmax.com/play_amongthem.md>
- Coworld package contract: <https://github.com/Metta-AI/metta/blob/main/packages/coworld/src/coworld/COWORLD_README.md>

## What To Improve First

The biggest early wins are reliable game-state signals.

1. **Connection and lifecycle.** Exit clearly when the websocket closes. Flush
   logs so failed episodes explain what happened.
2. **Protocol correctness.** Send Bitscreen button and chat packets only in the
   expected format.
3. **Perception.** Track phase, own position, room, role, visible tasks,
   bodies, players, meetings, and chat.
...
5. **Task execution.** Stop inside the task rectangle and hold action long
   enough.
6. **Meetings.** Parse voting slots, chat, and votes. Avoid self-votes and make
   vote choices explainable from observed evidence.

The best source references are `players/evidencebot_v2.nim` for stronger visual
play, `players/nottoodumb/nottoodumb.nim` for a smaller baseline, and
`players/ivotewell/ivotewell.nim` for the bundled Coworld smoke player.
...
Keep volatile frame state separate from episode memory. A transient pixel match
should not overwrite a reliable sighting.
